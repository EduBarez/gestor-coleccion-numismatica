...
exports.updateColeccion = async (req, res) => {
  try {
    const coleccion = await Coleccion.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!coleccion) return res.status(404).json({ error: 'Colección no encontrada' });
...
exports.deleteColeccion = async (req, res) => {
  try {
    ...
    if (!resultado) return res.status(404).json({ error: 'Colección no encontrada' });

    res.status(200).json({ message: 'Colección eliminada y monedas desvinculadas' });
  } catch (error) {
...
exports.agregarMonedasAColeccion = async (req, res) => {
    try {
      ...
      if (!Array.isArray(monedas)) {
        return res.status(400).json({ error: 'Se espera un array de IDs de monedas' });
      }
      ...
      res.status(500).json({ error: 'Error al agregar monedas', details: error.message });
    }
  };
