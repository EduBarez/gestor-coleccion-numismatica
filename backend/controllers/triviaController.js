...
exports.createPregunta = async (req, res) => {
  try {
    const { pregunta, opciones, respuestaCorrecta, monedaAsociada, periodo } = req.body;

    if (!pregunta || !opciones || opciones.length !== 4 || !respuestaCorrecta) {
    ...
    const nuevaPregunta = new Trivia({ pregunta, opciones, respuestaCorrecta, monedaAsociada, periodo });
...
exports.updatePregunta = async (req, res) => {
  try {
    const { id } = req.params;
    const preguntaActualizada = await Trivia.findByIdAndUpdate(id, req.body, { new: true });
