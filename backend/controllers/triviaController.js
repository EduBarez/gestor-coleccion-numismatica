...
exports.getPreguntas = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error al obtener las preguntas de trivia', details: error.message });
  }
};
...
exports.getPreguntaById = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error al obtener la pregunta', details: error.message });
  }
};
...
exports.createPregunta = async (req, res) => {
  try {
    ...
    if (!pregunta || !opciones || opciones.length !== 4 || !respuestaCorrecta || !periodo) {
      return res.status(400).json({ error: 'Faltan datos o las opciones no tienen exactamente 4 respuestas' });
    }

    if (!opciones.includes(respuestaCorrecta)) {
      return res.status(400).json({ error: 'La respuesta correcta debe estar dentro de las opciones' });
    }

    const nuevaPregunta = new Trivia({ pregunta, opciones, respuestaCorrecta, periodo });
    await nuevaPregunta.save();

    res.status(201).json(nuevaPregunta);
  } catch (error) {
    res.status(500).json({ error: 'Error al crear la pregunta de trivia', details: error.message });
  }
};
...
exports.updatePregunta = async (req, res) => {
  try {
    const { id } = req.params;
    const preguntaActualizada = await Trivia.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
...
exports.getPreguntasPorPeriodo = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error al obtener preguntas por periodo', details: error.message });
  }
};
...
exports.deletePregunta = async (req, res) => {
  try {
    ...
    if (!preguntaEliminada) return res.status(404).json({ error: 'Pregunta no encontrada' });
